...
assert.equal(evaluate(`recommendedRacerEvent().id`), 'heathcote_legal', 'new drivers are directed to the zero-Respect event');
assert.equal(evaluate(`raceEntryBlockers(eventById('heathcote_legal')).length`), 0);
assert.match(evaluate(`raceEntryBlockers(eventById('calder_test')).join(' ')`), /requires 3 Respect/);
assert.match(evaluate(`calendarCard(eventById('calder_test'))`), /spectator entry only/);
evaluate(`state.car.inWorkshop=true;state.car.unavailableUntilDay=3`);
assert.match(evaluate(`raceEntryBlockers(eventById('heathcote_legal')).join(' ')`), /unavailable in the workshop/);
evaluate(`state.car.inWorkshop=false;state.car.unavailableUntilDay=null`);
